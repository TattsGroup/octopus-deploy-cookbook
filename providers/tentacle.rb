...
action :register do
  ...
  register_instance = powershell_script "register-#{instance}-tentacle" do
    ...
    cwd tentacle_install_location
    code <<-EOH
      .\\Tentacle.exe register-with --instance "#{instance}" --server "#{server}" --name "#{display_name}" --apiKey "#{api_key}" #{register_comm_config(polling, port)} #{option_list('environment', environment)} #{option_list('role', roles)} #{option_list('tenant', tenants)} #{option_list('tenanttag', tenant_tags)} --console
